...
export default function Login() {
  const setAuth = useAuthStore((state) => state.setAuth); // ✅ zustand setAuth
  ...
  async function handleFakeLogin() {
    try {
      const res = await api.post('/members/auth/login', {
        username: 'test01',
        password: 'test1234!',
      });
      ...
      // 예시: 토큰 로컬스토리지 저장
      if (res.data.result) {
        const { accessToken, memberId } = res.data.result; // ✅ 구조분해 할당
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col items-center flex-grow justify-center gap-[10px]">
        {/* 로고 아이콘: 49 × 64.73(px) */}
        <img src={logo} alt="Eventory" className="w-[49px] h-[64.73px] object-contain" />
      ...
      <div>
        <button onClick={handleFakeLogin}>일반 테스트 로그인</button>
      </div>
      {/* 소셜/기타 로그인 버튼들 */}
      <section className="w-full px-5 mt-auto space-y-4">
        {/* 카카오 */}
        <button
          type="button"
      ...
      {/* 이메일 회원가입 링크 */}
      <div className="flex items-center gap-1 mt-4 mb-4 text-sm text-gray-600">
        <span>이메일로 회원가입</span>
