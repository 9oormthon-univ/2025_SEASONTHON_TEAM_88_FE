...
export default function PartyRecommend() {
  ...
  return (
    <main
      className={['w-[22.5rem] h-[46.25rem] mx-auto', 'bg-black text-white font-pretendard', 'flex flex-col'].join(' ')}
    >
      {/* 상단 다크 히어로 */}
      <header className="relative px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-4 overflow-hidden">
        ...
        <div className="relative z-[1]">
          ...
          >
            <img src={badgeGift} alt="" className="w-5 h-5" />내 파티 추가하기
          </button>
        </div>
      </header>
      ...
      <section className="flex-1 overflow-auto text-black bg-white rounded-t-3xl">
        <div className="px-4 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))] space-y-6">
                ...
                return (
                  <li key={it.key}>
                    ...
                    >
                      ...
                      <span className="text-[14px] text-gray-800 flex-1 text-left">{it.label}</span>
                      <span className="text-[12px] text-gray-500 mr-2">{it.price.toLocaleString()}원</span>
                      <span
                        className={[
                      ...
                      >
                        <span
                          className={['w-2.5 h-2.5 rounded', active ? 'bg-[#7F6BFF]' : 'bg-transparent'].join(' ')}
                        />
                      </span>
                    </button>
                  </li>
          ...
          <div className="relative overflow-hidden rounded-2xl">
            <img src={bannerImg} alt="회원 쿠폰 배너" className="object-cover w-full h-12" />
                ...
                {SIMILAR_ITEMS.map((it) => (
                  ...
                  >
                    ...
                    <div className="p-2">
                      ...
                      <div className="mt-1 text-[13px] font-semibold">{it.price.toLocaleString()}원</div>
                    </div>
                  </div>
                ))}
              </div>
